    ...
    if st.button("🔍 Ejecutar Diagnóstico Completo"):
        ...
        if st.button("🛠️ Forzar Creación"):
            ...
            if success:
                ...
                # Verificar qué grupos existen realmente
                unique_flags = df_draw['draw_propensity_flag'].unique()
                n_grupos = len(unique_flags)
                
                if n_grupos >= 2:
                    # Caso normal: hay ambos grupos (True y False)
                    comparison = df_draw.groupby('draw_propensity_flag')[['p_final_L', 'p_final_E', 'p_final_V']].mean()
                    
                    # Mapear índices correctamente
                    new_index = []
                    for flag in comparison.index:
                        if flag == False:
                            new_index.append('Sin Draw Propensity')
                        elif flag == True:
                            new_index.append('Con Draw Propensity')
                        else:
                            new_index.append(f'Flag: {flag}')
                    
                    comparison.index = new_index
